...
with col4:
    age_group = st.selectbox(
        "年齢ステージ 🎂", 
        ["子犬/子猫（成長期）", "成犬/成猫（1歳〜）", "シニア（高齢期）"]
    )

with col5:
    neutered = st.selectbox(
        "去勢・避妊手術 🩺", 
        ["手術している（避妊・去勢済み）", "手術していない（未手術）"]
    )
...
if st.button("AIに相談して計算する ✨"):
    ...
    計算の考慮事項（獣医学の基準）：
    - 成長期はカロリーを多く、シニアは控えめに計算してください。
    - 避妊・去勢済みの場合、代謝エネルギー（DER）の係数が低くなる（太りやすくなる）ため、給餌量を少し控えめに補正してください。
    
    回答の構成：
    1. フードのカロリーや年齢、手術の有無をすべて考慮した、推定される1日のごはんの量（グラム）を大きく提示
    2. そのグラム数になった理由（「去勢済みで太りやすいため〜」「成長期でたくさん栄養が必要なため〜」など）を優しく解説
